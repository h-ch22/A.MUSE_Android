package com.cj.amuse.culture.ui
...
fun CultureView(){
    ...
    val categories = stringArrayResource(id = R.array.CATEGORIES_CULTURE)
        ...
        ModalBottomSheetLayout(sheetContent = {
            ...
            Surface(modifier = Modifier.fillMaxSize()) {
                LaunchedEffect(key1 = true) {
                    helper.getContents(ContentsTypeModel.CULTURE){
                        for(i in 0 until if(ContentsHelper.cultureContents.size > 3) 3 else ContentsHelper.cultureContents.size){
                            bestContents.add(
                                ContentsHelper.cultureContents[i]
                            )
                        }
                ...
                Column(modifier = Modifier
                    ...
                    if(showProgress){
                        ...
                        Spacer(modifier = Modifier.weight(1f))
                    } else if(ContentsHelper.cultureContents.isEmpty()) {
                        Spacer(modifier = Modifier.weight(1f))
